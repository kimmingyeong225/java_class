...
public class MyBookStore {
	...
	public static void main(String[] args) {
		...
		while (flag) {
			...
			// 문자열 + 다음줄로 이동 처리
			String selectedNumber = sc.nextLine(); // 문자열을 받아주는 메서드 --> 한번에 줄바꿈까지 해주는 게 next

			if (selectedNumber.equals(SAVE)) {// 비교 == 이거 말고 .equals() 이거 쓰기
	...
	// 전체 삭제하기
	public static void deleteAll(Book[] books) { // 위에 deleteAll(books); 해줘야 됨
		System.out.println("-------- 전체 삭제하기-------");

		for (int i = 0; i < books.length; i++) {
			// Object --> null Object에 값이 없으면 null이 된다.
			books[i] = null;
		}
	}

	// 저장하기(하나의 북 객체를 저장)
	public static void save(Scanner sc, Book[] books) { // int --> 정수값이 반환
	...
	public static void readByTitle(Scanner sc, Book[] books) {
		System.out.println("------선택 조회 하기------");
		System.out.println(">>> 책 제목을 입력해주세여 <<<");
		String bookTitle = sc.nextLine();
		...
		// 문자열 --> equals
		// books[0].getTitle().equals() // 배열 안에 객체가 없다면 --> null pointer --> 방어적 토드
